...
def logout():
    ...
    return redirect(
        ...
        + urlencode(
            {
                "returnTo": url_for("auth", _external=True),
                "client_id": env.get("AUTH0_CLIENT_ID"),
            },
...
@app.route("/auth")
def auth():
    return render_template("auth.html", session=session.get('user'), pretty=json.dumps(session.get('user'), indent=4))
...
# logging.basicConfig(filename='/home/ubuntu/mathbenders.com/mathbenders/flask.log', level=logging.DEBUG)
...
def waitlist_apply():
    ...
    fr = 'charlie@mathbenders.com'
    subject = "Mana Games - Waitlist";
    text = str(data);
    text += " ip : "+str(ip)
    server = 'mathbenders.com'
